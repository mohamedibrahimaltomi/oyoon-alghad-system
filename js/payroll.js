const Payroll = {
  ...
  isMonthArchived(month) {
    return AppState.payrollArchive.some((x) => x.month === month);
  },

  canModifyArchivedMonth() {
    const role = AppState.currentUser?.role || "";
    return ["مدير النظام", "الإدارة"].includes(role);
  },
        ...
        <tr>
          ...
          <td>
            <div class="table-actions">
              ...
              <button class="btn btn-danger" onclick="Reports.exportEmployeePaySlipPDF('${row.employeeId}', '${month}')">طباعة كشف راتب</button>
            </div>
          </td>
        </tr>
      `).join("")
  ...
  openPayrollOverrideModal(month, employeeId) {
    if (this.isMonthArchived(month) && !this.canModifyArchivedMonth()) {
      openInfoModal("مقيد", "هذا الشهر مؤرشف ولا يمكن تعديله إلا بواسطة مدير النظام أو الإدارة.");
      return;
    }
  ...
  async approvePayrollMonth() {
    try {
      ...
      if (existing) {
        if (!this.canModifyArchivedMonth()) {
          openInfoModal("مقيد", "هذا الشهر مؤرشف ومُعتمد بالفعل. لا يمكن إعادة اعتماده أو تعديله إلا بصلاحية خاصة.");
          return;
        }
        await sbUpdate(TABLES.payrollArchive, existing.id, payload);
        await this.addLog("تعديل شهر مؤرشف", month);
      } else {
