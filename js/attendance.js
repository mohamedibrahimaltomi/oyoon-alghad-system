  ...
  async previewFingerprintImport() {
    try {
      ...
      AppState.fingerprintPreviewRows = rows.map((row) => {
        ...
        const employee = this.findEmployeeByImportedNo(employeeNo);

        const late = this.calculateLateMinutes(checkIn);
        const status = late > 0 ? "تأخير" : "حضور";

        const duplicate = AppState.attendance.some((x) =>
          String(x.employee_id) === String(employee?.id || "") && String(x.date) === String(date)
        );
        ...
        return {
          ...
          duplicate,
          errorText: !employee?.id ? "الموظف غير مطابق" : (!date ? "التاريخ غير موجود" : (duplicate ? "سجل مكرر" : ""))
        };
      });
        ...
        tbody.innerHTML = AppState.fingerprintPreviewRows.length
          ? AppState.fingerprintPreviewRows.map((row) => `
            <tr>
              ...
              <td>${safeText(row.employeeName || '-')}</td><td>${safeText(row.date)}</td>
              <td>${safeText(row.checkIn)}</td>
              <td>${safeText(row.lateMinutes)}</td>
              <td>${safeText(row.status)}</td>
              <td>${row.matched && !row.duplicate ? '<span class="status-pill success">مطابق</span>' : '<span class="status-pill danger">يوجد خطأ</span>'}</td>
              <td>${safeText(row.errorText || '-')}</td>
            </tr>
          `).join("")
