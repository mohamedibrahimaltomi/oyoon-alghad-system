const App = {
  ...
  bindNotifications() {
    ...
    $("notifyBtnHero")?.addEventListener("click", requestNotification);
  },
  ...
  bindGlobalSearch() {
    ...
    $("heroSearchFocusBtn")?.addEventListener("click", () => $("menuSearchInput")?.focus());
  },
  ...
  renderDashboard() {
    const todayRows = AppState.attendance.filter((x) => x.date === todayISO());
    ...
    if ($("todayAbsence")) $("todayAbsence").textContent = todayRows.filter((x) => x.status === "غياب").length;
    if ($("todayLate")) $("todayLate").textContent = todayRows.filter((x) => x.status === "تأخير").length;

    const month = currentMonthValue();
    let monthPayrollTotal = 0;
    if (typeof Payroll !== "undefined" && typeof Payroll.buildPayrollRows === "function") {
      monthPayrollTotal = Payroll.buildPayrollRows(month).reduce((sum, row) => sum + Number(row.net || 0), 0);
    }
    const remainingLoansTotal = (AppState.loans || []).reduce((sum, item) => sum + Number(item.remaining_amount || 0), 0);
